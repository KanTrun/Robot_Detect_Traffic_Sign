"""
Run serial with-sign monitor and report SIGN throughput/format stats.
"""
...
def run_monitor(port: str, baud: int, duration_sec: int):
    ...
    while time.time() < end:
        ...
        if m_sign:
            ...
            if len(sign_samples) < 50:
                sign_samples.append(line)
            continue
    ...
    sign_per_min = sign_total / (duration_sec / 60.0)
    conf_stats = {}
...
def main():
    parser = argparse.ArgumentParser(description="Measure with-sign stream behavior from ESP32-CAM serial")
    ...
    print(f"[RUN] with-sign monitor on {args.port}@{args.baud} for {args.duration_sec}s")
    ...
    print(f"[OK] sign_total={result['sign_total']} sign_per_min={result['sign_per_min']:.4f} malformed={result['malformed']}")
